...
def max_age_1_relation(group):
    ...
    return max([name.age for name in group if name.relations])
...
print(f"Max age: {max_age(my_group)}")
print(f"Mean age: {mean_age(my_group)}")
print(f"Max age of those that have one or more relations: {max_age_1_relation(my_group)}")
print(f"Max age of those that have one or more friends: {max_age_1_friend(my_group)}")