...
class Group:
    ...
    def number_of_connections(self, name):
        ...
        assert self.contains(name), "Person must be a part of the group"
    ...
    def connect(self, name1, name2, relation, reciprocal=True):
        ...
        """
        assert self.contains(name1) and self.contains(name2), "People must be a part of the group" 
        if reciprocal:
    ...
    def forget(self, name1, name2):
        ...
        if name1 in self.connections[name2]:
...
if __name__ == "__main__":
    ...
    my_group.connect("John", "Nash", "cousin")
